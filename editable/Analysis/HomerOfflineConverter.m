...
if nargin < 1
    pathname = uigetdir(cd, '/Users/michaelakent/Desktop/ZoomStudy/Pilot2022/HV/2022-03-29/output/');
end

%find all subfolders containing *.wl1 files (i.e. raw NIRx data)
try
    files = rdir(fullfile(pathname,'**','*.wl1'));
catch
    disp('Alert: rdir Matlab function not available. Trying to load single dataset instead.');
    files = dir([pathname filesep '*.wl1']);
    if isempty(files)
        disp(['Error: could not find *.wl1 data in ' pathname]);
        return;
    else
        files = files(1);
        files(1).name = [pathname filesep files(1).name];
    end
end
...
for subj=1:length(files)
    ...
    while (1)
        ...
        try
            if ~isempty(strfind(sline, 'SamplingRate'))
                sr = sscanf(sline, 'SamplingRate=%f\n');
            elseif ~isempty(strfind(sline, 'waveLength_N'))
                WLs = sscanf(sline, 'waveLength_N=%d\n');
            elseif ~isempty(strfind(sline, 'Wavelengths'))
                if WLs == 2
                    lambda = sscanf(sline, 'Wavelengths=[%d %d]', [1 2]);
                else
                    lambda = sscanf(sline, 'Wavelengths=[%d	%d %d %d]', [1 WLs]);
                end
            elseif ~isempty(strfind(sline, 'source_N'))
                nsrc_cfg = sscanf(sline, 'source_N=%d\n');
            elseif ~isempty(strfind(sline, 'detector_N'))
