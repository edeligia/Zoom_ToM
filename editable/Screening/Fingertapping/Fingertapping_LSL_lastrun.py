...
expName = 'Fingertapping'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\CulhmanLab\\Documents\\GitHub\\Zoom_ToM\\editable\\Screening\\Fingertapping\\Fingertapping_LSL_lastrun.py',
...
text_5 = visual.TextStim(win=win, name='text_5',
    ...
    languageStyle='LTR',
    depth=0.0);
sound_3 = sound.Sound('Audio//Counter', secs=-1, stereo=True, hamming=True,
    name='sound_3')
sound_3.setVolume(1)
...
text_2 = visual.TextStim(win=win, name='text_2',
    ...
    languageStyle='LTR',
    depth=0.0);
sound_2 = sound.Sound('Audio//Stop.wav', secs=-1, stereo=True, hamming=True,
    name='sound_2')
sound_2.setVolume(1)
...
Wait_BeginComponents = [text_9, key_resp]
...
# update component parameters for each repeat
sound_3.setSound('Audio//Counter', secs=3.5, hamming=True)
sound_3.setVolume(1, log=False)
# keep track of which components have finished
CountdownComponents = [text_5, sound_3, text_6, text_7, text_8]
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    # start/stop sound_3
    if sound_3.status == NOT_STARTED and tThisFlip >= 3.0-frameTolerance:
        # keep track of start time/frame for later
    ...
    if sound_3.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > sound_3.tStartRefresh + 3.5-frameTolerance:
            # keep track of stop time/frame for later
    ...
    # *text_7* updates
    if text_7.status == NOT_STARTED and tThisFlip >= 4.0-frameTolerance:
        # keep track of start time/frame for later
    ...
    # *text_8* updates
    if text_8.status == NOT_STARTED and tThisFlip >= 5.0-frameTolerance:
        # keep track of start time/frame for later
...
for thisTrial in trials:
    ...
    while continueRoutine:
        ...
        if text.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > text.tStartRefresh + textdur-frameTolerance:
                # keep track of stop time/frame for later
    ...
    # update component parameters for each repeat
    text_2.setText('Stop')
    sound_2.setSound('Audio//Stop.wav', secs=3.0, hamming=True)
    sound_2.setVolume(1, log=False)
    # keep track of which components have finished
    Stop_TrialComponents = [text_2, sound_2, text_3]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if sound_2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > sound_2.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
